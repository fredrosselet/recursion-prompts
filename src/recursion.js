...
var createArray = function(str) {
  if (str === '') {
    ...
    return [firstChar].concat(createArray(str));
  }
};
...
var alternateSign = function(array) {
  ...
  if (array[1] > 0) {
    return [Math.abs(array[0])].concat(-array[1]).concat(alternateSign(array.slice(2)));
  } else {
    return [Math.abs(array[0])].concat(array[1]).concat(alternateSign(array.slice(2)));
  }
};
...
var mergeSort = function(array) {
  ...
  collection.splice(collection.indexOf(smallest), 1);
  return [smallest].concat(mergeSort(collection));
};
